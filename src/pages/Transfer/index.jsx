...
import { Card, Typography, Upload, Button, List, Divider, notification } from "antd";
...
const TransferPaymentPage = () => {
  ...
  // Fungsi bantu untuk membuka notifikasi
  ...
  useEffect(() => {
    if (id) {
      getDataPrivate(`/api/orders/${id}`)
        ...
        .catch((err) => {
          openNotificationWithIcon("error", "Gagal Mengambil Data", "Gagal mengambil data pesanan.");
          console.error(err);
        });
    }
  }, [id]);
  ...
  const handleUpload = async () => {
    if (fileList.length === 0) {
      return openNotificationWithIcon("warning", "Bukti Tidak Ditemukan", "Harap unggah bukti pembayaran terlebih dahulu.");
    }

    if (!order) {
      return openNotificationWithIcon("warning", "Data Pesanan Tidak Ada", "Data pesanan tidak ditemukan.");
    }
    ...
    try {
      ...
      if (response && response.order) {
        ...
        openNotificationWithIcon("success", "Upload Berhasil", "Bukti pembayaran berhasil diunggah dan status diupdate.");
        setTimeout(() => {
          navigate("/pesanan");
        }, 2000);
      } else {
        openNotificationWithIcon("success", "Upload Berhasil", "Bukti pembayaran berhasil diunggah.");
        navigate("/pesanan");
      }
    } catch (error) {
      openNotificationWithIcon("error", "Upload Gagal", "Gagal mengunggah bukti pembayaran.");
      console.error(error);
    }
  };
        ...
        <Card title="Informasi Transfer" style={{ maxWidth: 600, width: "100%", margin: "0 auto" }}>
                ...
                <Card style={{ width: "100%", border: "1px solid #d9d9d9" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                      ...
                      <br />
                      <Text>No. Rekening: {account.accountNumber}</Text>
                      <br />
          ...
          {!order && (
            <>
              ...
              <Button type="primary" style={{ marginTop: 16 }} onClick={() => navigate("/pesanan")}>
                Lihat Pesanan Saya
              </Button>
            </>
          )}
